...
namespace WCFGenerator.RepositoriesGeneration.Core
{
    internal class VersionsRepositoryCodeGenerator : RepositoryCodeGeneratorBase
    {
        ...
        public override string GetFields()
        {
            ...
            {
                // Version filter
                var selectBy = (ScriptGenerator.GenerateSelectByToVersionTable(sqlInfo) + " {filter} ").SurroundWithQuotes();
                var selectByKeyAndSliceDateQuery = ScriptGenerator.GenerateSelectByKeyAndSliceDateToVersionTable(sqlInfo).SurroundWithQuotes();
                ...
                foreach (var method in RepositoryInfo.PossibleKeysForMethods)
                {
                    ...
                    var sqlV = ScriptGenerator.GenerateWhereVersions(parametrs, sqlInfo).SurroundWithQuotes();
                    var sqlA = ScriptGenerator.GenerateWhereVersionsWithAlias(parametrs, sqlInfo).SurroundWithQuotes();
                ...
                {
                    var sqlJoin = ScriptGenerator.GenerateWhereJoinPkVersion(sqlInfo).SurroundWithQuotes();
                ...
                if (RepositoryInfo.IsDeletedExist)
                {
                    ...
                    var andFilterV = ScriptGenerator.GenerateAnd(specialOption, sqlInfo.JoinVersionTableName ?? sqlInfo.VersionTableName).SurroundWithQuotes();
                    var andFilterA = ScriptGenerator.GenerateAndVersionsWithAlias(specialOption, sqlInfo).SurroundWithQuotes();
                ...
                if (RepositoryInfo.IsModifiedExist)
                {
                    ...
                    var filter = ScriptGenerator.GenerateAndVersionsWithAlias(specialOption, sqlInfo, "<=");
