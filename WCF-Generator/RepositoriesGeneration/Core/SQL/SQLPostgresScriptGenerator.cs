...
{
    internal static class SQLPostgresScriptGenerator
    {
        ...
        public static string GenerateUpdateWithoutTable(SqlInfo info)
        {
            var columns = info.TableColumns.Where(c => info.IdentityColumns.All(pk => pk != c)).ToList();
            return Update(info.TableName) + " "
                    + Set(columns, info.TableName) + " ";
        ...
        public static string GenerateUpsert(SqlInfo info)
        {
