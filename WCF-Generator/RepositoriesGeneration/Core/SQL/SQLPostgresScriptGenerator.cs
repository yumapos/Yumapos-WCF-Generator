...
namespace WCFGenerator.RepositoriesGeneration.Core.SQL
{
    internal class SQLPostgresScriptGenerator : ISqlScriptGenerator
    {
        ...
        {
            return Where(commonFilters, info.TableName) + AndTenantAndStoreRelated(info.TableName, info.TenantRelated, info.IsStoreDependent) + " " + WhereDates(datesFilters, info.TableName);
        }
        ...
        {
            return "NoCheckConstraint script was not generated";
        }

        public string GenerateCheckConstraint(SqlInfo sqlInfo)
        {
            return "CheckConstraint script was not generated";
        }
