...
namespace WCFGenerator.RepositoriesGeneration.Core.SQL
{
    internal class SqlScriptGenerator : ISqlScriptGenerator
    {
        ...
        public string GenerateInsertManyQueryTemplate(SqlInfo info)
        {
            ...
            var insertSql = !info.ReturnPrimaryKey
                ? InsertMany(columns, info.HiddenTableColumns.Select(c => c.Name), info.TableName)
                : InsertMany(columns, info.HiddenTableColumns.Select(c => c.Name), info.TableName, info.PrimaryKeyNames.First());
        ...
        public string GenerateNoCheckConstraint(SqlInfo sqlInfo)
        {
            return $"ALTER TABLE {sqlInfo.TableName} NOCHECK CONSTRAINT ALL";
        }

        public string GenerateCheckConstraint(SqlInfo sqlInfo)
        {
            return $"ALTER TABLE {sqlInfo.TableName} CHECK CONSTRAINT ALL";
        }
