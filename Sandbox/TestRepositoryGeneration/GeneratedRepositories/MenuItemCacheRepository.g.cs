...
namespace TestRepositoryGeneration.CustomRepositories.VersionsRepositories
{
	internal partial class MenuItemCacheRepository : TestRepositoryGeneration.Infrastructure.RepositoryBase
	{
		...
		private const string InsertQuery = @"DECLARE @TempItemIdTable TABLE (ItemId uniqueidentifier);INSERT INTO [RecipieItems]([RecipieItems].[ItemId],[RecipieItems].[ItemVersionId],[RecipieItems].[IsDeleted],[RecipieItems].[Modified],[RecipieItems].[ModifiedBy],[RecipieItems].[CategoryId],[RecipieItems].[TenantId]) OUTPUT INSERTED.ItemId INTO @TempItemIdTable VALUES(@ItemId,@ItemVersionId,@IsDeleted,@Modified,@ModifiedBy,@CategoryId,@TenantId);DECLARE @TempItemId uniqueidentifier; SELECT TempItemId = ItemId FROM @TempItemIdTable;INSERT INTO @[MenuItems]([RecipieItems].[MenuItemId],[RecipieItems].[MenuItemVersionId],[RecipieItems].[MenuCategoryId]) OUTPUT INSERTED.ItemId INTO @TempItemIdTable VALUES(@TempItemId,@MenuItemVersionId,@MenuCategoryId);SELECT ItemId FROM @TempItemIdTable;";
